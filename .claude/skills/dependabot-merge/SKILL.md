...
互換性の問題は利用側のビルドで顕在化しますが、脆弱性は黙って通ります。
...
2. 上流のリリースノート
   ...
   で引ける。セキュリティ修正が入っているか、新しい推移的依存が増えていないか
   （増えていれば信頼できる配布元か）を見る。破壊的変更・非推奨化・ライセンス変更は
   判断には使わず NOTES に回す。
...
| PR | 更新内容 | 確認したこと |
| --- | --- | --- |
| #12 | ktor 3.5.1 -> 3.6.0 (4 件) | 脆弱性なし / 上流差分 42 commits・不審な変更なし |
| #14 | detekt 1.23.8 -> 2.0.0 | 脆弱性なし / 上流差分に不審な変更なし ／ 注意: メジャーアップ。ルール既定値が変わり利用側の検出結果が変わる |

## 保留した PR

| PR | 更新内容 | 保留した理由 |
| --- | --- | --- |
| #15 | snakeyaml 1.30 -> 1.33 | GHSA-mjmj-j48q-9wg2 の対象範囲に新バージョンも入ったまま（patched は 2.0） |

## 確認できなかったこと

- #12: gradle-wrapper.jar はバイナリのため中身は未検証
- #15: 上流が bitbucket でタグ間差分は取得できていない
EOF
...
NOTES が「なし」でなければ、「確認したこと」欄の末尾に `／ 注意: ...` として併記します。
マージの判断には使っていませんが、BOM を使う側はビルドが壊れて初めて気づくことになるので、
バージョンを上げた記録と同じ場所に残してください。
